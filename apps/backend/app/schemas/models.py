...
class ResumeFetchData(BaseModel):
    ...
    processed_resume: ResumeDocument | None = None
...
class ImproveResumeData(BaseModel):
    ...
    # The structured comparison of the source against this proposal. The
    # same shape `POST /diff` returns, so one UI renders every diff surface.
...
class ImproveResumeConfirmRequest(BaseModel):
    ...
    improvements: list[ImprovementSuggestion]
    # Diff-row paths the user ticked. ``None`` accepts the whole preview,
    # which is the pre-partial-accept behaviour and stays the default.
    accepted_paths: list[str] | None = Field(default=None, max_length=2000)
...
class ResumeChange(BaseModel):
    ...
    path: str = Field(
        description=(
            "Dot+bracket path into the document, e.g. "
