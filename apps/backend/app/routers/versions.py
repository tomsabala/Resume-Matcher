...
async def get_version(version_id: str) -> VersionDetail:
    ...
    version = await db.get_resume_version(version_id)
    if version is None:
...
async def update_version(
    version_id: str, request: VersionUpdateRequest
) -> VersionDetail:
    ...
    try:
        version = await db.update_resume_version(version_id, updates)
...
async def delete_version(version_id: str) -> dict[str, str]:
    ...
    try:
        result = await db.delete_resume_version(version_id)
    except DatabaseBusyError:
        raise
    except Exception as exc:
