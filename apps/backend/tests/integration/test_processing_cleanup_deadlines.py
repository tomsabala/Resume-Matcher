...
async def test_stalled_finish_returns_bounded_and_keeps_owned_background_cleanup(
    ...
    record = await isolated_db.create_resume_atomic_master(
        content="Synthetic resume", processing_status="processing", workspace_id=workspace_id
    )
    resume_id = record["resume_id"]
    token = await isolated_db.claim_resume_processing(resume_id, workspace_id=workspace_id)
    ...
    caller = asyncio.create_task(
        resumes._finish_cancelled_processing(resume_id, token, workspace_id=workspace_id)
    )
    await entered.wait()
    for _ in range(3):
        caller.cancel()
    ...
    try:
        ...
        if newer_attempt:
            assert await isolated_db.claim_resume_processing(
    ...
    saved = await isolated_db.get_resume(resume_id, workspace_id=workspace_id)
...
async def test_late_claim_is_retired_after_cancelled_caller_returns(
    ...
    caller = asyncio.create_task(
        resumes._claim_processing(resume_id, workspace_id=workspace_id)
    )
    await entered.wait()
    caller.cancel()
    completed, _ = await asyncio.wait({caller}, timeout=0.15)
    ...
    try:
        ...
        assert background and any(not task.done() for task in background)
        if newer_attempt:
            await claim(resume_id, workspace_id=workspace_id)
    finally:
        release.set()
